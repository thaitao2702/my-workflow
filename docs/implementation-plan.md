# AI Coding Workflow — Implementation Plan (v2)
...
Component analysis docs (co-located with source code):
...
- `.workflow/` — Your runtime state, plans, project-specific rules. Separate from Claude's config. Can be committed to git or gitignored per preference.
- Component analysis docs (`*.analysis.md`) — Co-located with source code. Path IS the namespace (no naming collisions). Same pattern as `.test.tsx` files.
...
│   ├── Check affected components — load their .analysis.md files if they exist
...
Phase B: Component Intelligence Gathering (before plan creation)
...
│   │   ├── If {component}.analysis.md exists AND is current (last_commit matches) → load it
│   │   ├── If {component}.analysis.md exists BUT stale (code changed) → trigger /analyze (update mode)
│   │   └── If {component}.analysis.md does NOT exist → trigger /analyze (full mode)
│   │
...
│   │
│   └── Output: all affected components now have up-to-date .analysis.md files
...
│   │   ├── Relevant .analysis.md files (now guaranteed to exist + be current)
...
│   ├── Loads: .workflow/rules/planning/*.md (project-specific plan review rules)
...
affected_components: [component names with paths]
---
...
| File scope check (dimension 4) | Parallel phases modifying the same file = merge conflicts. This is the #1 cause of parallel execution failures. Learned from gsd's wave safety. | No file scope check (risk: merge conflicts in parallel execution) |
...
│   ├── Load relevant .analysis.md files for affected components
│   │   └── These should already exist (created during planning Phase B)
│   │       If somehow missing → trigger /analyze as fallback
...
│       ├── Relevant .analysis.md files (component knowledge)
│       ├── .workflow/rules/code/*.md (project-specific code quality rules)
│       └── Source files being modified (read by agent)
│
├── Step 2c: Phase Review (reviewer agent — model: sonnet)
│   ├── Loads: .workflow/rules/code/*.md (project-specific code review rules)
...
│   │   │   ├── Type safety (strongest available in the language)
│   │   │   ├── Error handling (propagated with context, not swallowed)
│   │   │   ├── Security basics (no hardcoded secrets, proper auth checks)
│   │   │   ├── No dead code, no commented-out code
│   │   │   ├── Naming clarity
│   │   │   └── Test quality (meaningful assertions, not just "doesn't throw")
│   │   │
│   │   └── Layer 3: Project-Specific (from .workflow/rules/code/*.md)
│   │       └── Rich markdown rules with examples, rationale, and exceptions
│   │
...
│   ├── Run staleness check (last_commit in .analysis.md vs current HEAD for that file)
...
| Co-located docs (`{Component}.analysis.md`) | Eliminates naming collisions (path IS the namespace). Same pattern as `.test.tsx` files. Git-friendly — code + doc changes in same directory. Discoverable via `**/*.analysis.md` glob. | Centralized directory (naming collisions for duplicate component names, harder to find) |
...
|-------|---------|-------|-----------------|
| **planner** | Creates phased execution plans from requirements | opus | Requirements, project-overview, relevant .analysis.md files, existing code |
| **reviewer** | Reviews plans and code against quality criteria | sonnet | Plan/code to review, .workflow/rules/{planning or code}/*.md, acceptance criteria |
| **executor** | Implements one task following TDD | sonnet | Task description, project-overview, .analysis.md files, source files, quality rules |
| **analyzer** | Deep component analysis | opus (full) / sonnet (update) | Component source, dependency .analysis.md files, project-overview, tests |
...
│ 3. Relevant Knowledge (.analysis.md)     │
...
- Project overview: ~1-4k tokens (always included)
- Component docs: ~300-500 tokens each (only relevant ones)
- Task/input: varies
- Rules: ~200-300 tokens per rule file
...
13. **Plan skill** — `.claude/skills/plan/SKILL.md`
14. **Jira skill** — `.claude/skills/jira/SKILL.md` + `jira_fetch.py` (ADF → markdown transformer)
15. **Pre-planning component analysis flow** — Auto-detect affected components → trigger `/analyze` if docs missing/stale
16. **Planner agent** — `.claude/agents/planner.md`
17. **Reviewer agent** — `.claude/agents/reviewer.md`
18. **Plan + phase templates** — Output formats (including Component Intelligence section)
19. **8-dimension review** — Automated plan quality check (loads `.workflow/rules/planning/`)
20. **Test** — Plan a real feature, verify plan quality

### Phase 5: Execute Plan (`/execute`)
21. **Execute skill** — `.claude/skills/execute/SKILL.md`
22. **Executor agent** — `.claude/agents/executor.md`
23. **State file management** — Create, update, resume from state.md
24. **Phase review flow** — Post-phase automated review (loads `.workflow/rules/code/`) + user review
25. **Doc-updater agent** — `.claude/agents/doc-updater.md` (Sonnet, significance assessment + conditional update)
26. **Parallel execution** — Group-based parallel dispatch
27. **Final reconciliation** — Targeted doc updates via staleness detection, not blind rewrite
28. **Test** — Execute a real plan, verify end-to-end flow

### Phase 6: Hooks & Safety
29. **Security gates** — `.claude/hooks/pre_tool_use.py`
30. **Quality checks** — `.claude/hooks/post_tool_use.py`
31. **Context monitoring** — `.claude/hooks/context_monitor.py`
32. **Hook registration** — `.claude/settings.json`

### Phase 7: Polish
33. **Resume protocol** — Test interrupt + resume across sessions
34. **Error handling** — Graceful failures (agent crash, test failure, etc.)
35. **Documentation** — Usage guide for the workflow itself

---

## Appendix: What We Took From Each Framework

| Concept | Source | How We Adapted It |
|---------|--------|-------------------|
...
| Progressive context loading | User's design | 3-level loading via markers (frontmatter / content / extra) |
| Leaf-to-top analysis | User's design | Recursive mode analyzes dependencies before parent |
...
| Plan format (medium detail) | All frameworks | Task goal + files + acceptance criteria + test requirements. No code in plan. |

## Appendix: Original Design Decisions

| Decision | Rationale |
|----------|-----------|
| Pre-planning component analysis | Analyze affected components BEFORE planning, not during execution. Plan with evidence, not assumptions. |
| Language-agnostic init | Discovery by manifest file detection, not framework-specific checklists. Works on any ecosystem. |
| Jira integration via Python script | ADF → markdown transformer. Heavy API response processed once outside context, clean output fed to planner. |
| Staleness-based analysis skipping | `last_commit` in frontmatter = natural flag. No explicit "already analyzed" tracking needed. Works across all contexts (planning, execution, ad-hoc). |
| Co-located component docs | `{Component}.analysis.md` alongside source. Path IS namespace — solves duplicate name problem. Same pattern as .test files. |
| Sonnet for doc updates (not Haiku) | Judgment step (NO/MINOR/MAJOR classification) saves money — most changes skip or get minor patches. |
| Rules as .md files, separated by purpose | Markdown rules are richer (examples, exceptions, rationale). Planning rules ≠ code rules — different directories for different reviewers. |
| Cost-aware model selection | Opus for planning/analysis (creative synthesis), Sonnet for execution/review/doc-updates (judgment + mechanical), cheapest model that can handle each task. |
| Project overview max 4k tokens | 2% of agent context is acceptable. Quality over compression — prevents architectural mistakes across all downstream agents. |
