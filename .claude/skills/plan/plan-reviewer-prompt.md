...
13. **Interface dependency coverage** — when Phase X's task describes calling, receiving, or importing code from Phase Y, verify: (a) the defining phase (Y) has an `interface_contracts` entry for that service, (b) the consuming phase (X) task description references the contract by phase and ID, (c) the contract includes: ID, expected class/module name, description, defined_in_task, and consumed_by_phases, (d) consumed_by_phases matches the actual consuming phases found in task descriptions, (e) **sequential ordering:** the defining phase is in an earlier execution group than every phase in consumed_by_phases — phases with interface dependencies MUST NOT be in the same parallel group, (f) **`interface_plan[]` is populated** with the six semantic fields for each entry (name, type, purpose, inputs_semantic, outputs_semantic, consumer_invariants), (g) **`interface_actual[]` is present and equal to `[]`** at plan-review time (it gets filled later by the producing executor — non-empty value at plan review = planner overreach), (h) **no exact type literals in `interface_plan[]`** (e.g., `{ field: number; optional?: string }`) UNLESS the entry cites a reused existing project type with `file_path:line_number` evidence in a Pattern Claim Verification table — pre-pinning exact shapes is planner overreach into execute-time implementation choices. Flag: cross-phase code dependencies with no contract declared. Flag: "calls the helper" or "receives via injection" without referencing a specific contract by phase and ID. Flag: producing and consuming phases in the same parallel group. Flag: any `interface_actual[]` that is non-empty at plan-review time. Flag: any `interface_plan[]` entry with exact type literals lacking evidence-row citation.
