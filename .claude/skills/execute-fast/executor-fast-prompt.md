...
| `{security_rules_path}` | `.claude/rules/security-gates.md` |
| `{quality_rules_path}` | `.claude/rules/quality-criteria.md` |
...
3. Implement so the task's **`Done when`** outcome becomes true. Touch only files in the task's `Files` list. Follow patterns from `§component-notes` and the project overview. Apply `{security_rules_path}` and `{quality_rules_path}` at all times — these override speed and are non-negotiable.
